"""
Runs adaptive energy-aware FL on SZ clients: adaptive selection and weighting.
"""
import os, math, time, json, numpy as np, torch, torch.nn as nn
from torch.utils.data import DataLoader, Dataset
from src.models.simple_gru import SimpleGRU
from src.fl.fedavg_baseline import clone_state, set_state, average_states, get_device, load_global_sets, make_eval_loader, client_indices
...
def run():
    """Runs adaptive FL with profiling and CSV logging."""
    device = get_device()
    ...
    print(f"Adaptive FL | rounds {rounds} | max_participants {max_participating}")
    ...
    for r in range(1, rounds + 1):
        ...
        if client_states:
            ...
            for k in keys:
                acc = None
                for st, wi in zip(client_states, weights):
                    val = st[k] * wi
    ...
    torch.save(global_state, os.path.join(OUT, "adaptive_state.pt"))
    print(f"Saved {OUT}/results.txt, adaptive_state.pt and {CSV_PATH}")
